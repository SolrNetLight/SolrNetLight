...
namespace SolrNetLight.Mapping {
    /// <summary>
    ...
    public class MappingManager : IMappingManager {
	    ...
	    public void Add(PropertyInfo property, string fieldName, float? boost) {
			...
			var declaringType = property.DeclaringType ?? property.GetType();
        ...
        public IDictionary<string,SolrFieldModel> GetFields(Type type) {
		    ...
		    return mappings
			    .Where(m => m.Key.DeclaringType == type)
        ...
        public SolrFieldModel GetUniqueKey(Type type) {
	        ...
	        var prop = uniqueKeys
		        .Where(k => k.Key.DeclaringType == type)
		        .Select(x => x.Value)
		        .FirstOrDefault();
